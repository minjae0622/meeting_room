...
<table border=0 cellpadding=0 cellspacing=0>
	...
	%>
	<tr>
		<td style="font-weight:bold;font-size:8pt;color: rgb(105, 105, 105); text-decoration: none;;text-align:center;height:10px;width:69px"><a href="javascript://" style="color:#696969;text-decoration:none;" onMouseOver="this.style.color='red'"  onMouseOut="this.style.color='#696969'" title=""><storng><%=classnm%></storng></a></td>
		<td>
			<table border=0 cellpadding=0 cellspacing=0 style="margin-left:-3px;border:1px solid #696969;background-color:#efefef;FILTER: alpha(opacity=70);table-layout:fixed">
				<tr>
					<% 
					for(int i=8; i< 19; i++){ 
						String strcolor = "";
						...
						for(int j=10;j<70;j+=10){
							...
							for(int x=0;x<room_reservation_data.length;x++){
								...
								if(ij>s_t && ij<=e_t && k==room_reservation_data[x].getRoom_num()){
										...
										strinfo = room_reservation_data[x].getInfo();
								}
							}
							
					%>
								<td style="cursor:pointer;width:5px;height:9px;<%=strcolor%>" title="<%=strinfo%>"></td>
					<%	}
					}%>
				</tr>
			</table>
